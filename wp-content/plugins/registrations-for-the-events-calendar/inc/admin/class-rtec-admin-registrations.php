	...
	public function the_events_overview() {
		...
		if ( $should_show_create_event_prompt ) {
			...
			<div class="rtec-notice rtec-box-shadow">
				<p><?php echo sprintf( __( "Looks like you there weren't any upcoming events allowing registration found. %sCreate an event%s to get started!", 'registrations-for-the-events-calendar' ), '<a href="' . admin_url( 'post-new.php?post_type=tribe_events' ) .'" class="button button-primary rtec-cta">', '</a>'); ?></p>
			</div>
			<?php
			$args = array(
				'posts_per_page' => $this->posts_per_page,
